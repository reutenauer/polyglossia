...
\begin{tabular}{lllll}
\toprule
\TX{afrikaans}  & divehi         & interlingua    & nko             & \TX{spanish}  \\
albanian        & \TX{dutch}     & \TX{italian}   & \TX{norwegian}  & swedish       \\
amharic         & \TX{english}   & japanese       & occitan         & \TX{syriac}   \\
\TX{arabic}     & \TX{esperanto} & kannada        & \TX{persian}    & tamil         \\
\TX{armenian}   & estonian       & khmer          & piedmontese     & telugu        \\
asturian        & \TX{finnish}   & \TX{korean}    & polish          & \TX{thai}     \\
basque          & \TX{french}    & \TX{kurdish}   & \TX{portuguese} & \TX{tibetan}  \\
\TX{belarusian} & friulian       & \TX{lao}       & romanian        & turkish       \\
\TX{bengali}    & \TX{gaelic}    & \TX{latin}     & romansh         & turkmen       \\
breton          & galician       & latvian        & \TX{russian}    & \TX{ukrainian}\\
bulgarian       & \TX{german}    & lithuanian     & \TX{sami}       & urdu          \\
\TX{catalan}    & \TX{greek}     & macedonian     & \TX{sanskrit}   & vietnamese    \\
coptic          & \TX{hebrew}    & \TX{malay}     & \TX{serbian}    & \TX{welsh}    \\
croatian        & \TX{hindi}     & malayalam      & \TX{slovak}     &               \\
\TX{czech}      & \TX{hungarian} & marathi        & \TX{slovenian}  &               \\
danish          & icelandic      & \TX{mongolian} & \TX{sorbian}    &               \\
\bottomrule
\end{tabular}
...
In version 1.46\new{v1.46}, support for Afrikaans and Belarusian has been added.
...
\begin{itemize}
	\item \TB{babelshorthands}\new{v1.1.1} globally activates \pkg{babel}
		...
		Afrikaans, Belarusian, Catalan, Czech, Dutch, Finnish, German, Italian,
		Latin, Mongolian, Russian, and Slovak. Please refer to the respective
