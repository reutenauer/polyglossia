...
\ie the number of characters that must be there at least at the beginning or end of a
...
so a word will only be hyphenated after at least two characters at the beginning and not
within the last three characters at the end.

To change this value, do the following after \pkg{polyglossia} is loaded and the languages
have been activated:
\begin{quote}
\begin{verbatim}
\renewcommand*{\⟨lang⟩hyphenmins}{{⟨l⟩}{⟨r⟩}}
\end{verbatim}
\end{quote}
with \meta{lang} replaced with the respective polyglossia language name (\eg \texttt{english}
for English), \meta{l} with the left threshold value (\eg \texttt{3}), \meta{r} with the right
threshold value. Note that this can be only changed once for all varieties of a given polyglossia
language currently.
