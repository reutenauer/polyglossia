...
\begin{quote}
\begin{verbatim}
\renewcommand*{⟨pattern⟩hyphenmins}{{⟨l⟩}{⟨r⟩}}
\end{verbatim}
\end{quote}
with \meta{pattern} replaced with the respective hyphenation pattern name (\eg \texttt{english}
for English or \texttt{ngerman} for German, new spelling),
\meta{l} with the left threshold value (\eg \texttt{3}), \meta{r} with the right
threshold value.
