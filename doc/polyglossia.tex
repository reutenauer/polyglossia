...
by Vafa Khaliqi (\textarabic{وفا خالقي}). Polyglossia also bundles three packages for calendaric 
...
\label{tab:lang}
%albanian
%amharic\footnotemark
%\TX{arabic}
%asturian\footnotemark
%bahasai
%bahasam
%basque
%brazil
...
\begin{tabular}{lllll}
\hline
albanian              & croatian       & \TX{german}   & norsk        & spanish       \\
amharic\footnotemark  & czech          & \TX{greek}    & nynorsk      & swedish       \\
\TX{arabic}           & danish         & \TX{hebrew}   & polish       & \TX{syriac}   \\
asturian\footnotemark & divehi         & hindi         & portuges     & \TX{thai}     \\
bahasai               & \TX{dutch}     & icelandic     & romanian     & turkish       \\
bahasam               & \TX{english}   & interlingua   & \TX{russian} & ukrainian     \\
basque                & \TX{esperanto} & irish         & samin        & \TX{usorbian} \\
brazil                & estonian       & italian       & sanskrit     & welsh         \\
breton                & \TX{farsi}     & latin         & scottish     &               \\
bulgarian             & finnish        & latvian       & \TX{serbian} &               \\
\TX{catalan}          & french         & \TX{lsorbian} & slovak       &               \\
coptic                & galician       & \TX{magyar}   & slovenian    &               \\
\hline
\end{tabular}
\caption{Languages currently supported in \pkg{polyglossia}}
\end{table}
\footnotetext{ New in version 1.0.1. 
This should be considered an experimental attempt to
port the package \pkg{ethiop}. Feedbacks are welcome.}
\footnotetext{ New in version 1.1.1. 
Language definition file provided by Kevin Godby and Xuacu Saturio.}
