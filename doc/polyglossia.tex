...
\TX{chinese}    & \TX{greek}     & macedonian     & \TX{sanskrit}   & \TX{uyghur}    \\
coptic          & \TX{hebrew}    & \TX{malay}     & \TX{serbian}    & vietnamese     \\
\TX{croatian}   & \TX{hindi}     & malayalam      & \TX{slovak}     & \TX{welsh}     \\
\TX{czech}      &                &                &                 &                \\
\bottomrule
\end{tabular}
...
\begin{itemize}
	...
	\item \Cmd\chinesenumeral produces Chinese numbering.
	          Depending on the ¦numerals¦ option in the Chinese language selection, this is either Arabic digit or Chinese
	          ideographic output.\\
...
\begin{itemize}
	\item Do not error, but warn, after language change due to non-updated aux file (\TXI{565}).
	      A helper macro \cmd\xselectlanguage\ is introduced for this.
	\item Fix catcode issue for Latin (\TXI{566}).
\end{itemize}

\subsubsection*{New features}
\begin{itemize}
\item Add support for Chinese (\TXI{65}).
\end{itemize}
