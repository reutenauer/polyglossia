...
asturian        & estonian       & kannada        & \TX{panjabi}    & tamil          \\
basque          & \TX{finnish}   & khmer          & \TX{persian}    & telugu         \\
\TX{belarusian} & \TX{french}    & \TX{korean}    & piedmontese     & \TX{thai}      \\
\TX{bengali}    & friulian       & \TX{kurdish}   & \TX{polish}     & \TX{tibetan}   \\
bosnian         & \TX{gaelic}    & \TX{lao}       & \TX{portuguese} & turkish        \\
breton          & galician       & \TX{latin}     & romanian        & turkmen        \\
bulgarian       & \TX{georgian}  & latvian        & romansh         & \TX{ukrainian} \\
\TX{catalan}    & \TX{german}    & lithuanian     & \TX{russian}    & urdu           \\
\TX{chinese}    & \TX{greek}     & macedonian     & \TX{sami}       & \TX{uyghur}    \\
coptic          & \TX{hebrew}    & \TX{malay}     & \TX{sanskrit}   & vietnamese     \\
\TX{croatian}   & \TX{hindi}     & malayalam      & \TX{serbian}    & \TX{welsh}     \\
\TX{czech}      &                &                & \TX{slovak}     &                \\
\bottomrule                 
\end{tabular}
...
\subsection{panjabi}\label{panjabi}
\paragraph*{Options:}
  \begin{itemize}
...
\TA{Philipp Stephani}, \TA{Niranjan Tambe}, \TA{Takuji Tanaka}, \TA{Osman Tursun}, \TA{Keno Wehr}, \TA{Dominik Wujastyk},
