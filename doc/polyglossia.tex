...
\begin{tabular}{lllll}
\hline
albanian     & \TX{dutch}     & \TX{italian}  & occitan        & \TX{syriac}    \\
amharic      & \TX{english}   & japanese      & piedmontese    & tamil          \\
\TX{arabic}  & \TX{esperanto} & kannada       & polish         & telugu         \\
\TX{armenian}& estonian       & khmer         & \TX{portuguese}& \TX{thai}      \\
asturian     & \TX{farsi}     & \TX{korean}   & romanian       & \TX{tibetan}   \\
\TX{bahasa}  & \TX{finnish}   & \TX{kurdish}  & romansh        & turkish        \\
basque       & \TX{french}    & \TX{lao}      & \TX{russian}   & turkmen        \\
\TX{bengali} & friulan        & \TX{latin}    & \TX{sami}      & \TX{ukrainian} \\
breton       & galician       & latvian       & \TX{sanskrit}  & urdu           \\
bulgarian    & \TX{german}    & lithuanian    & scottish       & vietnamese     \\
\TX{catalan} & \TX{greek}     & \TX{magyar}   & \TX{serbian}   & \TX{welsh}     \\
coptic       & \TX{hebrew}    & macedonian    & slovak         &                \\
croatian     & \TX{hindi}     & malayalam     & \TX{slovenian} &                \\
czech        & icelandic      & marathi       & \TX{sorbian}   &                \\
danish       & interlingua    & nko           & spanish        &                \\
divehi       & irish          & \TX{norwegian}& swedish \\
\hline
\end{tabular}
...
\subsection{bahasa}\label{bahasa}
\textbf{Options}:
\begin{itemize}
	\item \TB{variant}\new{v1.45} = \textit{indonesian} (=~indonesia) or malay (=~melayu)
\end{itemize}
...
for Arabic, Farsi, Urdu, Turkish and Bahasa provide a localized version of ¦\Hijritoday¦.
