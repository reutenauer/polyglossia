...
Czech typesetting requires some specifics which can be addressed by means of external packages.
Please load these yourself if required:

\begin{itemize}
	\item If a word with a hard hyphen (such as \emph{je-li}) is hyphenated at this hyphen,
	      a second hyphenation character is to be inserted at the beginning of the line that
	      follows the hyphenation (\cmd\splithyphens\ switch in \pkg{babel}\footnote{%
	      Since the \cmd\splithyphens\ feature interacts badly with many packages and is thus
          discouraged even in \pkg{babel} itself, it is not implemented in \pkg{polyglossia}.}).
	      In \pkg{polyglossia}, this can be manually achieved by activating babelshorthands
	      and using the ¦"=¦ shorthand (see below) rather than a dash character (¦je"=li¦).
	      If you are using \LuaTeX, split hyphens are inserted automatically if you load the
	      \pkg{luavlna} package (similarly as with \cmd\splithyphens\ in \pkg{babel}, but without
	      the inadvertent side effects).

    \item Single-letter words must not occur at line ends. This can be achieved by using the
          package \pkg{xevlna} (for \XeTeX) or \pkg{luavlna} (for \LuaTeX).
          
    \item Hyphenation at page breaks must be omitted. This must be taken care of manually.
\end{itemize}
