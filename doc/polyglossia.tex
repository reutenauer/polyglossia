...
\usepackage{color}
\usepackage{xspace,fancyvrb,booktabs,tabularx}
...
divehi       & icelandic      & marathi      & \TX{slovak}     & \TX{welsh}\\
\bottomrule
\end{tabular}
...
\begin{itemize}
	\item \TB{babelshorthands}\new{v1.1.1} globally activates \pkg{babel}
          ...
          Catalan, Czech, Dutch, Finnish, German, Italian, Latin, Mongolian, and Russian.
...
\paragraph*{Options:}
	...
	These variants refer to different spelling conventions. The \texttt{classic}
	and the \texttt{medieval} variant do not use the letters \emph{U} and
	...
	\texttt{hyphenation} option if the default style does not fit your needs.
	...
	\texttt{hyphenation=liturgical} in any case.
	\begin{table}
	\centering
	\begin{tabular}{ll}
	\hline
	\emph{Language variant} & \emph{Default hyphenation style} \\
	\hline
	classic & classic \\
	medieval & modern \\
	modern & modern \\
	...
	\end{table}
	\item \TB{ecclesiasticfootnotes} = true or \emph{false}
	...
	as default even for the \texttt{ecclesiastic} variant.

	Note that this option is only possible if Latin is the main language of your
	document.
	\item \TB{usej} = true or \emph{false}
	...
	disappeared from most Latin publications. So \emph{false} is the default
	...
	\item \TB{capitalizemonth} = true or false
	...
	liturgical books they are lowercase. So \emph{true} is the default value for
	the variants \texttt{classic}, \texttt{medieval}, and \texttt{modern},
	whereas \emph{false} is the default value for the \texttt{ecclesiastic}
	variant.
	\item \TB{babelshorthands} = true or \emph{false}

	Enable shorthands inherited from \pkg{babel-latin} and the \pkg{ecclesiastic}
	package as explained in table
	\ref{tab:latin-shorthands}.
	\begin{table}
	\begin{tabularx}{\linewidth}{lX}
	\hline
	\emph{Shorthand} & \emph{Meaning} \tabularnewline
	\hline
	\verb:"<: & « (left guillemet) \tabularnewline
	\verb:">: & » (right guillemet) \tabularnewline
	\verb:": & If no other shorthand applies, \verb:": before any letter
	character defines an optional break point allowing further break points
	within the same word (as opposed to the \cmd\- command). \tabularnewline
	\verb:"|: & the same as \verb:":, but also possible before non-letter
	characters \tabularnewline
	\verb:'a: & á (a with acute), also available for é, í, ó, ú, ý, ǽ, and \'œ
	\tabularnewline
	\verb:'A: & Á (A with acute), also available for É, Í, Ó, Ú, \'V, Ý, Ǽ, and
	\'Œ \tabularnewline
	\hline
	\multicolumn{2}{@{}p{\linewidth}}{\emph{The following shorthands are only
	available for the \texttt{medieval} and the \texttt{ecclesiastic} variant.}}
	\tabularnewline
	\verb:"ae: & æ (ae ligature), also available for œ
	\tabularnewline
	\verb:"Ae: & Æ (AE ligature), also available for Œ
	\tabularnewline
	\verb:"AE: & Æ (AE ligature), also available for Œ
	\tabularnewline
	\verb:'ae: & ǽ (ae ligature with acute), also available for \'œ
	\tabularnewline
	\verb:'Ae: & Ǽ (AE ligature with acute), also available for \'Œ
	\tabularnewline
	\verb:'AE: & Ǽ (AE ligature with acute), also available for \'Œ
	...
	\hline
	\end{tabularx}
	\caption{\label{tab:latin-shorthands}Latin shorthands}
	\end{table}
	\item \TB{prosodicshorthands} = true or \emph{false}

	Enable shorthands for prosodic marks (macrons and breves) very similiar to
	those provided by \pkg{babel-latin} using the \texttt{withprosodicmarks}
	modifier. The available shorthands are explained in table
	\ref{tab:latin-prosodic}.

	Note that the active `=' character used for macrons will cause problems with
	commands using \texttt{key=value} interfaces, e.\,g.
	\verb:\includegraphics[scale=2]{...}:. Use \Cmd{\shorthandoff\{=\}} before
	...
	\emph{Shorthand} & \emph{Meaning} \tabularnewline
	\hline
	\verb:=a: & ā (a with macron), also available for ē, ī, ō, ū, and ȳ
	\tabularnewline
	\verb:=A: & Ā (A with macron), also available for Ē, Ī, Ō, Ū, V̄, and Ȳ. Note
	that a macron above the letter V is only displayed if your font supports the
	Unicode character \texttt{0304} (\emph{combining macron}). \tabularnewline
	\verb:=ae: & a͞e (ae diphthong with macron), also available for a͞u, e͞u, and
	o͞e. Note that macrons above diphthongs are only displayed if your font
	supports the Unicode character \texttt{035E} (\emph{combining double
	macron}). \tabularnewline
	\verb:=Ae: & A͞e (Ae diphthong with macron), also available for A͞u, E͞u, and
	O͞e. \tabularnewline
	\verb:=AE: & A͞E (AE diphthong with macron), also available for A͞U, E͞U, and
	O͞E. \tabularnewline
	\verb:^a: & ă (a with breve), also available for ĕ, ĭ, ŏ, ŭ, and y̆. Note that
	a breve above the letter y is only displayed if your font supports the
	Unicode character \texttt{0306} (\emph{combining breve}). \tabularnewline
	\verb:^A: & Ă (A with breve), also available for Ĕ, Ĭ, Ŏ, Ŭ, V̆, and Y̆. Note
	that breves above the letters V and Y are only displayed if your font
	supports the Unicode character \texttt{0306} (\emph{combining breve}).
	\tabularnewline
	\hline
	\end{tabularx}
	\caption{\label{tab:latin-prosodic}Latin prosodic shorthands}
	\end{table}
	\end{itemize}
