...
\begin{itemize}
	\item \TB{babelshorthands}\new{v1.1.1} globally activates \pkg{babel}
		...
		Catalan, Czech, Dutch, Finnish, German, Italian, Latin, Mongolian, Russian,
		and Slovak. Please refer to the respective language descriptions
...
\begin{itemize}
	...
	\begin{shorthands}
		...
		\item[¦"¦] If no other shorthand applies, \verb:": before any letter
		...
		\item[¦"|¦] the same as \verb:":, but also possible before non-letter
			characters
		...
		\item[¦'A¦] for Á (A with acute), also available for É, Í, Ó, Ú, \'V, Ý, Ǽ,
			and \'Œ
	\end{shorthands}
	The following shorthands are only available for the \texttt{medieval} and the
	\texttt{ecclesiastic} variant.
	\begin{shorthands}
	...
	\begin{shorthands}
		...
		\item[¦=A¦] for Ā (A with macron), also available for Ē, Ī, Ō, Ū, V̄, and Ȳ.
			...
			supports the Unicode character \texttt{0304} (\emph{combining macron}).
		...
		\item[¦\textasciicircum a¦] for ă (a with breve), also available for ĕ, ĭ,
			...
			your font supports the Unicode character \texttt{0306} (\emph{combining
			breve}).
		\item[¦\textasciicircum A¦] Ă (A with breve), also available for Ĕ, Ĭ, Ŏ,
			...
			if your font supports the Unicode character \texttt{0306} (\emph{combining
			breve}).
	\end{shorthands}
\end{itemize}
