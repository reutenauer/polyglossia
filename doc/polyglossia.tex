...
\begin{itemize}
	...
	\item \xpgoption{extension-t} The subtag indicating transformation between languages or scripts
	      (\eg, \texttt{en-t-ja}: English transformed to Japanese)
	\item \xpgoption{extension-u} The subtag indicating additional locale information (\eg \texttt{ar-u-nu-latn}:
	      Arabic with Latin Numbering)
	\item \xpgoption{extension-x} The private usespace subtag (\eg \texttt{la-x-classic} for classic Latin)
...
\end{itemize}
If a subtag is not defined for a given language, and empty string is returned.
...
\begin{itemize}
	\item Introduce \cmd\BCPdata, a standardized way to access language information
