...
\begin{itemize}
	\item \TB{babelshorthands}\new{v1.1.1} globally activates \pkg{babel}
          ...
          Catalan, Dutch, Finnish, German, Italian, Mongolian, and Russian.
...
where \texttt{$\langle$language$\rangle$} is a \pkg{polyglossia} language name, or
\displaycmd{\ifbabellanguageloaded\{$\langle$babelname$\rangle$\}\{$\langle$true$\rangle$\}\{$\langle$false$\rangle$\}}{\ifbabellanguageloaded}
where \texttt{$\langle$language$\rangle$} is a \pkg{babel} language name.
